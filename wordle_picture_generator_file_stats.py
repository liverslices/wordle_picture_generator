...
all_words = read_lines_from_file("valid-wordle-words.txt")
...
for solution in all_words:
    ...
    for guess in all_words:
        pattern = word_into_pattern(guess, solution)
        for i in list(range(0,6)):
            # print(f"DEBUG: Comparing {pattern} and {pattern_to_match[i]}")
            if pattern == pattern_to_match[i]:
                pattern_solution[i] = guess
                # input(f"Found word! Guess \"{guess}\" matches solution \"{solution}\" with the pattern {pattern}")
    ...
    if pattern_found:
        ...
        not_possible += 1
        not_possible_words.append(solution)

    print(f"{(possible + not_possible)}/{len(all_words)} Possible: {possible} Not possible: {not_possible}")
